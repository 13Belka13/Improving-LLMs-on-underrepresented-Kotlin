...
from src.constants.python_constants import BUILT_IN_FUNCTIONS, KEYWORD_DICT
from src.constants.python_patterns import (
    TOKEN_PATTERN, COMMENT_PATTERN, IMPORT_PATTERN,
    STRING_LITERAL_PATTERN, NUMBER_LITERAL_PATTERN, ANNOTATION_PATTERN
)


class PythonCodePreprocessor(CodePreprocessor):
    def __init__(self):
        super().__init__()
        ...
        while i < len(tokens):
            ...
            if token.isidentifier():
                if token in self.keyword_dict:
                    ...
                    normalized_tokens.append('DUNDER_IDENTIFIER')
                ...
                if i > 1 and normalized_tokens[-1] in {'FUNCTION_NAME', 'VARIABLE_NAME', 'CLASS_NAME',
                                                       'PRIVATE_IDENTIFIER', 'LOOP_VARIABLE'} and \
                        normalized_tokens[-2] != 'def' and normalized_tokens[-2] != 'class':
